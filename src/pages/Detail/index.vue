            ...
            <div class="chooseArea">
              ...
              >
                ...
                <dd
                  ...
                  "
                  :class="{ active: spuAttrSaleValue.isChecked == '1' }"
                  :key="spuAttrSaleValue.id"
                >
                  {{ spuAttrSaleValue.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              ...
              <div class="add">
                <!--  这里加入购物车，进行路由跳转之前，发请求会把购买产品的信息通过请求的形式 
                        通知服务器，服务器进行相应的存储
                 -->
...
export default {
  ...
  methods: {
    ...
    async addToShopCart() {
      /* 
        1.在点击加入购物车这个按钮时，做的第一件事，就是将参数带给服务器，通知服务器购物车的产品是什么
        2.发送请求后，需要知道这次请求时成功还是失败。如果成功，则进行路由跳转；如果失败，则提示用户。
      */
      //  this.$store.dispatch就相当于调用仓库中的action的函数，返回值也是action中的返回值。
      //console.log(ret,typeof(ret));//用了await之后，ret就变成String了；而用之前则是Promise。await等待的是成功的结果————fulfilled的Promise的result

      try{
        await this.$store.dispatch("addOrUpdateShopCart", {skuId:this.$route.params.skuid,skuNum:this.skuNum});
        // 成功之后，进行路由的跳转
        // 路由跳转时还需要将产品的信息传递给加入购物车页
        // 简单的参数如skuNum，通过query形式给路由组件传递过去
        // 产品信息的数据，比较复杂的[如skuInfo]，通过会话存储进行传递。（不持久化，关闭浏览器会消失）
        // 不管是本地存储还是会话存储，一般存的都是字符串。
    ...
    changeCheck(spuAttrSaleValue, arr) {
      arr.forEach((item) => {
        // 先把所有值的isChecked置位
        item.isChecked = "0";
      });
      // 被点击的属性值的isChecked置为'1'
      spuAttrSaleValue.isChecked = "1";
    },
    // 加入购物车之前处理产品个数（杜绝用户的非法输入）
    chengeSkuNum(e) {
  ...
  computed: {
    ...
    skuImageList() {
      /*     console.log('this.skuInfo',this.skuInfo);
        console.log('Detail中的this.skuInfo.skuImageList',this.skuInfo.skuImageList); */
