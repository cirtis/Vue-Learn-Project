...
export default {
  ...
  beforeCreate() {
    ...
    // console.log("beforecreated addressList", this.addressList);
  },
  data() {
    // 收集买家的留言
    return{
  ...
  created() {
    // console.log("created addressList", this.addressList);
  },
  beforeMount() {
  ...
  mounted() {
    ...
    // console.log("mounted addressList", this.addressList);
  },
  computed: {
    ...mapState({
      addressList: (state) => {
    ...
    // 将来提交订单中的地址
    userDefault() {
      // console.log('TMD',this.addressList);
      // 查找数组当中符合条件的元素返回为最终结果
      let obj = this.addressList.find((address) => address.isDefault == 1);
      // console.log("123", obj); //为什么console执行了两次？？ 跟虚拟DOM的渲染事件有关？
      // console.log("计算属性中的addressList", this.addressList);
      return obj || {};//来个兜底的，不然obj可能是undefined
    },
  },
  methods: {
    // 修改默认地址
